...
- 发布/部署证据：提交 `4317cdd` 的 GitHub Actions quality run `34555617183` 的 contracts、API、Web、browser-e2e、release-notes 和 API/Web GHCR job 均成功；文档对齐提交 `9516370` 的同一质量流程也全部成功。切换前备份 `/home/syin/study-backups/20260911T024702Z`，回滚副本在 `/home/syin/study-source-backups/20260911T024702Z-tutor-diagram/`；项目 `verify-restore.sh` 因临时数据库初始化缺失未计为通过，随后使用同一备份在一次性显式建库容器中恢复成功，报告 39 张 PostgreSQL public 表、888 个 MinIO 文件。
- Ubuntu 当前最终固定 `ghcr.io/yubinhong/aistudy-api:sha-9516370` 与 `ghcr.io/yubinhong/aistudy-web:sha-9516370`；API/Web digest 分别为 `sha256:38cab483774f95bff840db81637f26e0f92d3abe9b6e9c5e17e3b52e6e74a2d2`、`sha256:6c03f48c7f2fad44a2bd3c076c0ef574abfad8b982ce44e8b3bae8792f3ce941`，OCI revision 均为 `9516370e0b79f7bd516d1008bf1b3dceb1839e50`。API/Web health、OpenAPI `0.17.5` Tutor 路由、`0039_smartedu_curriculum_source (head)`、五个 API/worker 容器、MinIO 无宿主端口和容器内 `_provider_message_content`/`question_image` 源码检查通过。
...
- Ubuntu：首次备份 `/home/syin/study-backups/20260911T015531Z`，回滚副本 `/home/syin/study-source-backups/20260911T015531Z-capture-media-preserve/`；最终文档 revision 切换前再次备份 `/home/syin/study-backups/20260911T020415Z`，回滚副本 `/home/syin/study-source-backups/20260911T020415Z-final-docs/`。远端最终固定 `sha-9543088`，API/Web OCI revision 均为 `954308850cd2b5369f5201f8fa462f6f2b485e66`；迁移 `0039_smartedu_curriculum_source (head)`、API/Web health、四个 worker、近期错误计数 0、MinIO 私有端口和容器内 worker 源码检查通过。
...
- 范围：将当前工作区的 SmartEdu 目录选择、资源 ID 下载、私有 PDF 草稿加载、`0039_smartedu_curriculum_source` 迁移和既有解析队列先部署到 Ubuntu，作为 GHCR 发布前的功能验证阶段；没有导入真实教材或修改儿童学习事实。
...
- 未执行（实现阶段记录）：真实 SmartEdu 目录/PDF、平台登录/条款、版权/教研签核、真实 Provider 质量/成本、真实家长浏览器、四端设备和公网/商业生产验收；GHCR 发布与拉取式切换已由下节完成。

## 2026-09-10 SmartEdu GHCR 拉取式修复部署

- 发布：提交 `a7454a89bf7e83b847671de8b46460ecdb422840` 已推送 `master`，annotated tag `v0.17.3` 已推送。tag quality run `34425077122` 的 contracts、API、Web、browser-e2e 和两个 GHCR 发布 job 均成功；Android run `34425077166`、master quality run `34425029726` 也成功。
- 载荷：Ubuntu 固定 API `ghcr.io/yubinhong/aistudy-api:sha-a7454a8`，RepoDigest 为 `sha256:b3ec72fb3949bc14987b2407ef9499fb2be025f578f0c49652b0c68470563fcf`；Web `ghcr.io/yubinhong/aistudy-web:sha-a7454a8`，RepoDigest 为 `sha256:ca2be9204745ae50f934db6af69e68841a0389d0195c5b95bf3d16ece6c275a6`。两镜像 OCI revision 均为完整提交 `a7454a89bf7e83b847671de8b46460ecdb422840`。
- 备份：切换前 `/home/syin/study-backups/20260910T012805Z` 已完成；`verify-restore.sh` 隔离恢复报告 `postgres_public_tables=39`、`minio_snapshot_files=739`。切换前远端 `.env`/Compose 副本保存在 `/home/syin/study-source-backups/20260910T012805Z-ghcr/`，未回传本地或输出密钥。
- 发布步骤：同步当前 `infra/compose/compose.yml`，恢复应用服务 `pull_policy: always`，将远端 `.env` 的 API/Web 固定到上述 `sha-a7454a8`，依次通过 `config --quiet`、`docker compose pull` 和 `docker compose up -d --no-build`；迁移容器 `Exited (0)`。
- 验收：API/Web 本机与 `192.168.1.4` LAN health 均返回 200，API 返回版本 `0.17.3`；Alembic 为 `0039_smartedu_curriculum_source (head)`，API/Web/四个 worker running；两个 SmartEdu 路由、`smartedu` provider、16 MiB 目录响应上限、50 MiB PDF 上限均在运行容器中确认；最近 5 分钟应用错误计数为 0，MinIO `9000` 未映射宿主端口。
- 未执行与回滚：真实 SmartEdu 目录/PDF、平台登录/条款、版权/教研签核、真实 Provider 质量/成本、真实家长浏览器、四端设备、staging/production 仍未验收。回滚应用优先固定此前已验证的 GHCR `sha-f6ae9a2` 并重新拉取/启动，数据库保留 `0039` 且不执行 downgrade；旧应用不兼容时恢复 `sha-a7454a8` 并以前向修复处理。
...
- [ ] PLAN-0016/0017/0018、ADR-0021/0022/0023：Ubuntu 已实施 `0.17.3`/`0039` 的 PDF-only、SmartEdu 私有草稿加载、错题 closeout/ReviewAttempt、私有原页、多模态知识图谱、家长批准、批准知识点推荐和孩子端原页入口；真实 118 页 PDF 已完成机器解析并批准，备份恢复、迁移头、健康和私有端口烟雾通过。仍须完成正式版权/教研、真实 Provider 质量/成本、Ubuntu 真实账号/iPad/完整设备验收和个人信息门禁后才可勾选。
