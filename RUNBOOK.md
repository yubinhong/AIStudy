...
- 发布/部署证据：提交 `4317cdd` 的 GitHub Actions quality run `34555617183`、文档对齐提交 `9516370` 和最终对齐提交 `374a798` 的质量流程均成功。切换前备份 `/home/syin/study-backups/20260911T024702Z`，回滚副本在 `/home/syin/study-source-backups/20260911T024702Z-tutor-diagram/`；项目 `verify-restore.sh` 因临时数据库初始化缺失未计为通过，随后使用同一备份在一次性显式建库容器中恢复成功，报告 39 张 PostgreSQL public 表、888 个 MinIO 文件。
- Ubuntu 当前最终固定 `ghcr.io/yubinhong/aistudy-api:sha-374a798` 与 `ghcr.io/yubinhong/aistudy-web:sha-374a798`；API/Web digest 分别为 `sha256:65c2f170f3e8dfc704db19eca486c2388fdd7b88a7c14fb683fe39aebf03944c`、`sha256:887844a5f41c5bdb46f9b17d0fa479afecf0eb8f2ac509d21fc4924553f5f121`，OCI revision 均为 `374a798c46978b7f19cd94176a3c698ddd1cd81d`。API/Web health、OpenAPI `0.17.5` Tutor 路由、`0039_smartedu_curriculum_source (head)`、五个 API/worker 容器、MinIO 无宿主端口和容器内 `_provider_message_content`/`question_image` 源码检查通过。
