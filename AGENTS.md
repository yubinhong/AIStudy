...
- 当前 P0/P1 基础脚手架已存在：`apps/`、`services/`、`packages/`、`evals/`、`infra/compose/`、三类锁文件、CI 与隔离 Chromium 登录态 E2E；本地 API/OpenAPI 已前移至 `0.17.4`，Ubuntu 仍运行 `0.17.3`，迁移头均为 `0039_smartedu_curriculum_source`。Profile/Learning/Capture/Identity/VerifiedQuestion/Tutor/Report/Export/Mistake/Review、SmartEdu 家长教材目录/PDF 草稿加载和 Ubuntu 自用 Compose 均有实现/验收记录；SmartEdu 默认免配置，页面在需要时接收一次性 JSON（`access_token` 必填，`mac_key`/`diff` 可选），本地签名自动化已完成，需发布、部署和真实 PDF 验证后才算运行修复。显式 `math/chinese`、subject-aware 教材及语文确定性 Content/Attempt/Review 首个纵向切片已部署 Ubuntu，当前应用载荷通过 GHCR `sha-a7454a8` 提供。古诗抽查只允许版本化目录中标题、连续诗句和全部选项都通过的内容，Nova 9 已覆盖安装并完成 12 轮全六首无作答抽查；正式内容签核、Ubuntu 真实账号浏览器和其他完整设备回归仍未完成。PLAN-0018 的私有原页预览、分批多模态教材分析、整本知识图谱、家长审核和已审核知识点/具体练习推荐已部署，真实 Provider/PDF/设备质量验收仍待完成。
...
- ADR-0018 已接受以“App 携带 Session → API 有界流式校验/转发 → 私有 MinIO”替代 ADR-0010/0014 的预签名直传。目标 OpenAPI 不返回对象存储 URL，MinIO `9000` 不向宿主/LAN 暴露，且删除 `OBJECT_STORAGE_PUBLIC_ENDPOINT_URL`；本地 `0.17.4` 与 Ubuntu `0.17.3` 均保持新链路，最终设备回归仍待完成。
