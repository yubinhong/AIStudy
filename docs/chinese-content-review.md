...
没有完整证据的原创条目必须保持 `pending_owner_review`。它们仅可作为项目原创演示与自动化夹具，不得宣传为正式教材、版权已审核内容或教研定稿。家庭私有教材不进入公共课程发布，只能在已发布快照和家长授权声明的家庭范围内使用。
...
这些条目已由 `0033_chinese_poem_spot_check` 标记为 `retired`，保留历史 Attempt/Review 供审计，但不会进入孩子题库。它们尚未收到具名教研/版权签核，不能恢复为正式课程。

## 当前代码门禁与测试

- 原创内容只有在 `status=approved`、`license_status=cleared`，且 `source.review` 具备 `approved`、`project_owner`、审核时间和权利凭证 SHA-256 时，才允许孩子读取、提交和进入复习队列。
- 家庭私有教材古诗使用已发布快照和家长授权声明作为家庭范围门禁，`private_authorized` 内容只对绑定家庭/孩子可见；不跨家庭复用原文。
- golden scorer 已覆盖拼音、生字、词语、句子、阅读、背诵、表达和古诗八类技能。Owner 签核后，应以新的内容 revision 写入审核摘要和时间；既有 Attempt 始终引用旧 revision，不得回写或重判。
