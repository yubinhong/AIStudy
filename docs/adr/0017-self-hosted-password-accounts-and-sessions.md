...
6. 默认凭据有效期间，Compose 的 Web 引导入口只允许绑定本机回环地址。该账号登录后只允许调用 `me`、修改密码和退出；其他页面/API 返回稳定的 `password_change_required`，不得读取家庭、孩子、学习或图片数据。
7. 首次修改密码成功后清除 `must_change_password`、撤销全部引导会话并签发新会话。部署者完成改密后才能显式把 Web 暴露到家庭局域网。非 local/self-hosted 引导模式发现默认凭据仍有效时必须拒绝启动或保持数据接口关闭。
