...
- 替代/被替代：客户端预签名直传决策于 `2026-07-17` 被 `ADR-0018` 替代；私有 MinIO、S3 兼容 Adapter、随机对象键、客户端无长期密钥和生命周期事实由 ADR-0018 继承；ADR-0015 继续限制原图与脱敏副本外发
- 批准记录：项目 Owner 明确选择本地 MinIO、S3 兼容 Adapter、私有 Bucket 与短期预签名上传；客户端不得持有存储密钥。

> 历史状态说明：本文保留 `0.5.0`～`0.8.0` 预签名直传的决策与验证事实，不再代表目标上传架构。当前代码仍按本文运行，完成 `PLAN-0012` 前不得把 ADR-0018 目标误报为已实现。
