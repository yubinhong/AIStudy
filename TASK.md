...
- [x] 完成本地质量门槛及 Ubuntu 备份、隔离恢复、`0037` 迁移、`0.17.1` 部署和真实题库核验；157 道错误题退役，只保留咏鹅、画、悯农（其二）、江南、古朗月行、风共 21 道，Attempt/Review 迁移前后均为 1。
- [ ] Nova 9 首次重连后实际进入古诗抽查，驻留的旧 Flutter 页面仍从进程内列表打开“日积月累 / 种瓜得瓜”，证明服务端退役不能使已加载页面自动失效。客户端现改为每次点击古诗入口都重新读取 API；第二次响应已撤销时阻断旧题。定向 7 项、全量 72 项和 Analyze 通过，Release APK 已构建；设备在覆盖安装前断开，待重连后安装并完成多轮不作答抽查。
- [ ] 提交、创建 `v0.17.1` annotated tag，并推送 GitHub；核验远端 refs 与标签构建。

回滚：不 downgrade `0037` 或重新批准错误题；保留 Attempt、Review、教材和审核事实，以匹配应用前向修复。设备验收不提交答案。
