# PLAN-0052 README 简化与 v0.17.6 发布

- 状态：`IN_PROGRESS`
- 创建：`2026-09-11`
- 关联：`TASK-0012`、`CHANGELOG.md`、`README.md`

## 目标

把仓库首页改成普通家庭用户能快速读懂的项目介绍，保留安装、安全和开发文档入口；优先尝试用真实 iPad Flutter 画面补充 README，设备截图不可用时使用仓库已有的安全界面预览；将当前已验证的孩子端修复和文档变更提交为 `v0.17.6`，创建并推送 `master` 与版本 tag。

## 验收

- [x] README 不再展示服务器规格、模型评测、迁移编号和 CI 细节等维护者信息；功能、适用范围、安装入口和隐私提示清楚可读。
- [x] 已尝试从 iPad 取得截图；当前设备原生截图不支持，Xcode 调试连接也未建立，因此 README 使用已有的界面预览和 synthetic 拍题图，不冒充真机照片。
- [x] CHANGELOG 有唯一的 `v0.17.6 - 2026-09-11` 区块，提交和 annotated tag 指向同一版本内容。
- [x] Flutter 相关质量检查、README 图片检查和 Git 差异检查通过；`master` 与 `v0.17.6` 推送成功。

## 回滚

README 和截图可随提交回退；版本 tag 不移动、不覆盖，若发布流程失败只修复提交后创建新的补丁 tag。不得删除或回滚数据库、Capture、学习事实或 Ubuntu 数据。
...
- 计划 ID：`PLAN-0046`
- 关联：`PLAN-0044`、`PLAN-0045`、`TASK-0012`、`ADR-0029`、`SECURITY.md`、`RUNBOOK.md`
- 状态：`IN_PROGRESS`
- 优先级：`P1 / API / SECURITY / SELF-HOSTED DEPLOYMENT`
- Owner：Codex（实现、测试和发布）；项目 Owner（提供真实教材/平台账号验证授权）
- 创建：`2026-09-10`

## 目标、边界与里程碑

修复真实 SmartEdu 教材详情可解析、但私有 CDN 因占位 `X-ND-AUTH` 返回 `400 InvalidArgument` 并被 API 转成 `503` 的问题。默认免配置；家长在页面需要时粘贴一次性 JSON，至少包含 `access_token`，`mac_key` 和 `diff` 可省略，API 按每个受信 PDF URL 兼容生成对应 MAC。凭据只临时保存在当前浏览器标签页的 `sessionStorage` 以便刷新和连续加载，手动清除、退出登录或关闭标签页后清除；不得进入数据库、日志、提交或镜像。无凭据、凭据无效或 CDN 拒绝鉴权时返回稳定可操作错误，本地 PDF 上传继续可用；同一孩子/资源的重试沿用幂等键，已有结果在下载前直接回放。

- [x] M1 — 增加有界一次性 JSON 凭据解析、URL 绑定 MAC 签名、匿名默认路径、错误分类和单元回归；不新增第三方依赖。
...
新增一个可选的向后兼容请求字段 `smartedu_credentials_json`，不修改数据库结构；未填写凭据时目录仍可读取，严格 PDF 资源返回 `smartedu_source_requires_authentication`，不再伪装成一般 `503`。JSON 长度、字段、换行和时钟差均有界校验；Web 只把凭据写入当前标签页 `sessionStorage`，服务端仅在当前请求内存中使用，手动清除或退出登录后删除，关闭标签页由浏览器清除；请求日志、教材记录和错误不含原值。SmartEdu 同一孩子/资源的客户端幂等键也保存在当前标签页，服务端先检查已有导入回执，避免可回放请求重新下载 PDF。回滚时恢复上一 API/Web 镜像并移除页面输入/请求字段，保留 PostgreSQL/MinIO/Redis 卷和所有教材、解析及学习事实，不执行数据库 downgrade。
...
在家长教材后台增加国家中小学智慧教育平台的可选目录和“加载为草稿”操作，复用现有私有 PDF、SHA-256、解析队列、原页审核和发布链路。适配器只接受受控的资源 ID；默认免配置，严格私有资源需要时由家长在页面粘贴一次性 JSON（`access_token` 必填，`mac_key`/`diff` 可选），不向 Web 返回第三方 PDF 直链，不把未审核内容提供给孩子端、Tutor 或 Provider；本轮不抓取或提交真实教材正文，不改变英语范围，不推送 GHCR。
...
- 计划 ID：`PLAN-0036`
- 关联：`TASK-0012`、`PLAN-0024`
- 状态：`COMPLETE`
- 优先级：`P1 / WEB / DATA LIFECYCLE`
- Owner：Codex（实现与验证）；项目 Owner（已确认两个孩子的清理范围）
- 创建：`2026-08-30`

## 目标、边界与里程碑

移除家长首页的“语文技能报告”展示及对应请求；“今日需要关注”只展示上海自然日今天到期的开放错题，不把今天之前已经逾期的历史项继续列为今日事项。完整错题与学习事实仍保留在既有学习记录/复习边界，不能通过 UI 筛选伪装成已删除。用户确认后，历史记录清理由单独的备份后运维操作完成，不在应用迁移中静默删除；本次范围为该家庭两个孩子的全部学习历史，保留账号、孩子档案、教材/审核事实、设备设置和审计记录。

- [x] M1 — 定位首页技能报告、到期错题查询和 180 天学习记录保留边界；确认现有 `due_only=true` 返回所有 `due_at <= now` 的开放错题。
- [x] M2 — 删除首页技能报告组件及对应 API 请求，增加上海自然日“仅今天到期”纯函数并接入首页计数、列表和趋势摘要。
- [x] M3 — Web 单元、格式、Lint、类型和生产构建通过；回归覆盖上海日边界、长期逾期、今天到期、未来和无效日期。
- [x] M4 — 同步 PRD、架构、测试、任务与变更记录，明确后端能力保留、首页只隐藏逾期项。
- [x] M5 — Ubuntu Web 部署、备份与隔离恢复已通过；用户确认两个孩子后，删除两名孩子的任务/会话/作答、拍题及私有对象、语文作答/复习、错题/复习/推荐、讲解、教材图片分析和导出快照，所有目标历史表复核为 0。保留 2 个孩子档案、3 个账号、3 份教材、3 个快照、184 条已审核语文内容和审计记录；API/Web/全部 worker 已恢复并健康。

## 回滚与风险

UI 回滚只恢复首页组件和旧筛选，不改数据库。数据清理不可通过应用回滚恢复，必须先备份并验证恢复能力；不得读取或输出题目正文、孩子姓名、账号、图片或 Provider 内容，也不得将“所有学习历史”解释为仅清理可重算的复习状态。
