...
CHINESE_CURRICULUM_PAGE_INSTRUCTIONS = (
    ...
    "exercise. Reserve kind poem strictly for classical Chinese poetry (古诗/古典诗词), not "
    "nursery rhymes, children's songs, modern poems, prose, textbook chants, tongue twisters "
    "or other rhyming text; classify those as passage. Use short visible boundary markers. For "
    "kind poem only, title must be the poem's actual title and lines must copy every visible "
    "verse line in order so a parent can review private next-line practice; for all other kinds "
    "lines must be []. Do not reconstruct missing or unclear characters. "
