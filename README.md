...
AIStudy 是一个面向家庭自托管的开源学习助手。当前以小学数学为主线，把家庭教材范围、错题采集、分步讲解、错题沉淀、到期复习和家长反馈组织成可追溯的学习闭环。

项目强调家庭数据隔离、人工确认、来源可追溯和可替换 AI Provider，不把模型输出直接当作标准答案或永久掌握度事实。

## 当前状态

当前仓库版本为 API/OpenAPI `0.13.0`，数据库迁移头为 `0030_learning_history_retention`。Ubuntu 单家庭自用 Compose 已完成迁移、健康检查和备份恢复验证，但这不等同于公网或商业生产发布。

| 能力 | 状态 |
| --- | --- |
| 家长 Web | 账号登录、孩子/账号管理、教材上传与审核、学习记录、导出与删除已实现 |
| 孩子 Flutter | iOS/Android 登录、数学学习桌、拍题、题目确认、分级提示、错题与复习已实现 |
