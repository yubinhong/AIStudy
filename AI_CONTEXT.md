...
- 2026-08-29 已修复古诗抽查把《剪窗花》等童谣/韵文作为古诗的问题：Provider `poem` 只作为候选，`classical-poem-catalog.v1` 以标题和至少两句连续诗句确定性验证，发布、读取和提交均失败关闭；同教材重发先退役旧派生题。Ubuntu `0037` 将 157 道错误题前向退役并保留全部历史，只留下咏鹅、画、悯农（其二）、江南、古朗月行、风共 21 道；Attempt/Review 数量不变。Nova 9 首次复查暴露迁移前驻留页面仍能打开进程内旧题，Flutter 因此增加每次进入前重新读取当前题库并在撤销时阻断的门禁；72 项测试和 Release 构建通过，待设备重连覆盖安装、多轮抽查和 `v0.17.1` 发布。
