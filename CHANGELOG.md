...
- 2026-08-29：修复语文“古诗抽查”把《剪窗花》等儿歌、童谣或现代韵文作为古诗的问题。Provider 分类仅作候选，服务端以 `classical-poem-catalog.v1` 验证标题和连续诗句，并在发布、读取和提交处失败关闭。新增 `0037_classical_poem_gate`，前向退役旧错误派生题而不删除 Attempt、Review、教材或审核事实；同一教材重新发布会先退役旧题，再只恢复通过门禁的题目。语文教材页级 Prompt 升至 v4；Flutter 每次打开古诗抽查前重新读取当前题库，避免驻留页面继续使用已退役旧题。Ubuntu 已部署 `0.17.1/0037`；Nova 9 新 APK 多轮抽查及 `v0.17.1` tag 尚待完成。
