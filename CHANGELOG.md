...
- 修复完成数学题后点击“返回学习桌”因返回路由层级错误导致学习桌被一并弹出、iPad 出现黑屏的问题。
- 增加学科选择、学习桌和讲解页之间的嵌套导航回归；真实 iPad 集成测试已自动完成解题、完成和返回流程并通过，Release 修复包已重新构建、签名、安装并成功启动。

### README

- 简化仓库首页说明，改为普通用户更容易理解的功能、使用方式和注意事项。
- 增加家长工作台和拍题示例图片；图片标注为界面预览，不包含真实家庭数据。

### iPad 图片入口权限恢复

- 数学拍题与语文看图选择改为不请求完整照片元数据，减少 iOS 不必要的照片库权限依赖。
- 相机或照片权限被拒绝时显示明确恢复说明并可直达 App 系统设置；系统限制、无可用相机和重复请求不再统一显示“暂时无法打开图片入口”。
- 图片入口失败日志只记录稳定错误码和入口类型，不记录图片路径、内容或用户数据。
...
- 修复 image-analysis worker 在分析后删除 Capture 原对象导致家长学习记录图片不可用的问题；现由既有 Capture 生命周期策略统一清理，保留原图 24 小时、OCR failure 7 天或家长保存窗口。
- 旧 worker 已删除的历史对象无法从 MinIO 恢复；部署后新拍题记录会在保留窗口内继续通过 Household-scoped 私有媒体流显示。
- 修复提交 `9a02425` 已通过质量/浏览器检查、GHCR 发布并部署 Ubuntu；旧对象不回填，真实新拍题显示仍待人工验收。
...
- 2026-09-05：服务端交付改为 GitHub Actions 构建 GHCR 镜像。`quality` 在契约、API、Web 和隔离浏览器门槛通过后发布 API/Web 的 amd64/arm64 镜像、版本/提交标签、provenance 和 SBOM；Pull Request 不发布。Compose 的迁移、API、四个 worker 和 Web 不再本地构建，改为拉取可固定版本的镜像。本轮未推送、未产生首批 GHCR 镜像，也未切换 Ubuntu。

- 2026-09-04：家长后台“学习记录”拆分为“数学学习记录”和“语文学习记录”两个子菜单。数学页面继续展示已确认数学题和分步讲解；语文页面新增家长专用答题记录，展示孩子提交的答案、答对/答错、错误时的正确答案、耗时和复习安排。新增 OpenAPI 查询路径和 PostgreSQL/内存仓储投影；英语暂不加入，未改变孩子端评分、复习和教材审核边界，次日按白名单部署 Ubuntu。

- 2026-09-04：修复 iPhone 首次安装后本地网络权限未登记、登录页直接报 `errno 65: No route to host` 的问题。iOS 现在在健康检查前使用 `NWConnection` 连接用户填写的实际家庭服务器，等待系统授权决定后再请求 `/healthz`；不发送业务数据、不增加第三方依赖或 Bonjour 服务。iPhone 11 修复包已覆盖安装，授权后 Ubuntu 实际收到 `192.168.1.100` 的健康请求并返回 200。

- 2026-09-03：家长后台完成首轮视觉改版。侧栏升级为深石墨工作导航，顶部当前孩子/服务/账号信息和主内容区重新分层；学习记录页采用紧凑筛选带、记录汇总和独立空状态，修复“时间范围”文字被裁切及无记录时大面积空白。桌面 `1280×800` 与手机 `390×844` 登录态布局断言、完整 Chromium E2E、Web 37 项单元测试、格式、Lint、类型和生产构建通过。代码提交 `be3bd70` 已推送并定向部署 Ubuntu Web；API、数据库、对象存储和 worker 未重启。
