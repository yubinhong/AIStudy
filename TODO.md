...
- TODO-215：任务执行体验重构 — 当前推荐只保存来源题，孩子端进入任务会退化为通用拍题，且多个任务同时展示造成低干扰学习界面失效；孩子端入口已临时隐藏。重新开放前须定义单一任务队列、开始后直接加载指定题目、作答/提示/完成/跳过的状态迁移、当天容量、逾期策略、家长撤销和离线一致性，并完成真机 E2E。
