...
当前仓库已有 P0/P1 依赖清单、三类锁文件、核心测试和 CI 草案。API 的 Household/认证/学习/Capture/可信 Tutor/周报/导出、Mistake/Review closeout、教材 PDF-only 私有原页/多模态知识图谱、作答四态/推荐审批、Web/Flutter 入口、SQLite 任务位置与离线 Attempt/任务终态队列、服务端任务位置/容量/未来日期/撤销保护和 Compose 已验证；Android/iOS 构建及 PostgreSQL/MinIO 恢复已有记录。本地和 Ubuntu API/OpenAPI 均为 `0.17.1`、迁移头 `0037_classical_poem_gate`。剩余是 Nova 9 本次古诗抽查、正式语文内容签核、真实 PDF/Provider 质量成本和完整设备 E2E 发布门槛。

2026-08-29 古诗题库门禁与 Ubuntu 部署：API 定向回归先后为 `51 passed`、`52 passed`，非集成全量通过；Ruff 格式/检查、Mypy（63 source files）、Flutter Analyze/全量 `72 passed`、OpenAPI `0.17.1` 解析、Alembic 单 head/从零离线 SQL 和 `git diff --check` 通过。本机没有可用 PostgreSQL/Docker，故真实前滚改在 Ubuntu 备份后验证。备份 `/home/syin/study-backups/20260829T070622Z` 隔离恢复为 39 张 public 表和 534 个 MinIO 文件；源码备份为 `/home/syin/study-source-backups/20260829T072000Z`。停止全部写入端后前滚 `0036 -> 0037`，approved 古诗题从 178 收敛为 21，157 道前向退役；只保留咏鹅、画、悯农（其二）、江南、古朗月行、风，Attempt/Review 均由迁移前 1 保持为 1。API/Web `/healthz`、四个 worker、运行时 `0.17.1`、`classical-poem-catalog.v1`、页级 Prompt v4 和 MinIO 私有端口通过；运行时确认《剪窗花》拒绝、《咏鹅》接受。Nova 9 首次重连后从迁移前驻留页面实际打开“日积月累 / 种瓜得瓜”，定位为页面内存仍持有旧 API 结果；Flutter 现于每次点击古诗入口重新读取当前内容，回归覆盖旧《剪窗花》刷新为《咏鹅》及撤销后阻断。Release APK 为 65,447,061 bytes，SHA-256 `e9e16d393620840c8c5d8f78f2bdbfb6cdbed6379e852c0605acc28118be70a9`；设备在覆盖安装前断开，多轮实际抽查尚未执行，不能描述为真机通过。
