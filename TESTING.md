...
- 验证通过：API Capture 定向 `7 passed`、API 非集成 `261 passed, 32 deselected`、Ruff/Mypy；Web 媒体代理定向 `1 passed`、全量 `39 passed`、Prettier/ESLint/TypeScript/build；OpenAPI/JSON Schema/运行时路由、契约引用闭合和 `git diff --check` 通过。随后已按本文件顶部记录部署 Ubuntu；真实账号/设备浏览器仍待执行。
...
2026-08-29/30 古诗题库门禁、Ubuntu 与 Nova 9 验收：API 古诗定向最终 `29 passed`，Ruff 和 Mypy（63 source files）通过；完整非集成套件除午夜边界既有失败外其余 256 项通过。该失败由宿主 `date.today()` 仍为 UTC 8 月 29 日、服务端上海日期已为 8 月 30 日造成，服务端正确以 `409` 拒绝前一天任务，与本次古诗代码无关且未以重跑掩盖。Flutter Analyze/全量 `72 passed`，OpenAPI `0.17.1`、Alembic `0038` 单 head/从零离线 SQL和 `git diff --check` 通过。本机没有 PostgreSQL/Docker，真实前滚在 Ubuntu 备份后验证。第一次备份 `/home/syin/study-backups/20260829T070622Z` 隔离恢复为 39 张 public 表/534 个 MinIO 文件；`0037` 将 178 道 approved 候选收敛为六首 21 道并退役 157 道。Nova 9 首次复查暴露驻留页面旧题，Flutter 改为点击前实时刷新；覆盖安装 APK（65,447,061 bytes，SHA-256 `e9e16d393620840c8c5d8f78f2bdbfb6cdbed6379e852c0605acc28118be70a9`）并保留登录态。第一轮真正古诗仍含 42 个童谣干扰项，故新增 v2 全选项门禁和 `0038`。第二次备份 `/home/syin/study-backups/20260829T220038Z` 隔离恢复为 39 张 public 表/528 个 MinIO 文件；迁移后非古诗选项 `42 -> 0`，Attempt/Review 保持 `1/1`。API/Web、四个 worker、运行时 `classical-poem-catalog.v2` 和 MinIO 私有端口通过。Nova 9 迁移后连续 12 轮覆盖咏鹅、画、悯农（其二）、江南、古朗月行、风，题干与正确下一句连续，三个选项均为古诗句；未选择或提交答案，数据库 Attempt/Review 仍为 `1/1`。远端 `master` 与 annotated `v0.17.1^{}` 均解析到 `e44a2b1`；GitHub `quality` run `33278258569` 和 Android run `33278259768` 均为 `success`，公开 Release 含 3 个 ABI APK、`SHA256SUMS` 与 `BUILD-INFO.txt`。

2026-08-25 语文教材分析 Schema/413 兼容修复与 Ubuntu 部署：Provider/教材作业定向 `30 passed`，API 非集成全量 `254 passed`，相关 Ruff 和 Mypy（62 source files）通过。合成回归覆盖 `observation` 与 `type`/`description` 替代字段、四页 413 递归二分及单页 413 失败保留，验证日志只含计数、不含教材内容。发布前备份 `/home/syin/study-backups/20260825T141449Z` 复跑隔离恢复通过（39 张 PostgreSQL public 表、363 个 MinIO 文件）；API/Web LAN health、Alembic `0036` current/head、运行时 `newapi`、MinIO 无宿主端口、远端/容器源码哈希、页级 Prompt v3 和 synthetic 丢弃 smoke 均通过。首次隔离恢复临时容器启动竞态及 BuildKit Docker Hub IPv6 超时均在切换运行代码前收口。运维显式重排既有真实语文作业后，第 3 次尝试完成全部页级调用并暴露整书替代字段；整书 Prompt 升至 `chinese-curriculum-book-consolidation.v3` 后第 4 次尝试完成 `118/118` 页，初始状态 `needs_review`，数据库计数为 10 个章节、12 个 draft 知识点、38 条古诗边界证据且无错误码。2026-08-26 最终部署后只读复核显示外部审核事实已更新为知识图谱和 12 个知识点全部 `approved`，解析/部署本身没有自动批准。未读取或保存 Provider 原始响应/教材正文；尚未执行 token/费用、正式版权/教研、真实账号浏览器和设备验收。
...
| Web E2E | `cd apps/web && pnpm test:e2e:install && pnpm test:e2e` | 用户流程变更/P1 门槛 | 通过（2026-09-03；隔离 synthetic API，`1 passed`；Ubuntu 真实账号未执行） |
...
| Compose 完整启动 | `docker compose -f infra/compose/compose.yml pull && docker compose -f infra/compose/compose.yml up -d` | API/数据/跨模块变更 | 通过（2026-09-09；Ubuntu 固定 `sha-f6ae9a2`，备份恢复、digest/OCI revision、迁移、API/Web LAN health、媒体 OpenAPI 路由和四个 worker 已核验） |
...
- [x] OpenAPI/Schema 结构检查通过；运行时 path/method/version 与规范对比纳入本轮复核，SDK 生成器仍未选择。
- [x] 现有 Household/认证/学习/Capture/Tutor/删除边界的家庭授权、幂等/离线和高风险自动测试通过；不包含 ADR-0020 新能力。
- [ ] PrivacySanitizer、用户外发确认、单 Provider、云视觉 Schema/人工确认和临时脱敏副本删除门禁已有 synthetic/自动回归；真实儿童图片、自动视觉检测器和真实 Provider 质量未验收前，不得宣称完整门禁通过。
...
- [ ] PLAN-0016/0017/0018：本地与 Ubuntu `0.17.2`/`0038` 代码/部署、真实 118 页 PDF 机器解析/批准和隔离 Chromium 已完成；仍需正式版权/教研、真实 Provider 质量/成本、Ubuntu 真实账号、完整设备 E2E 和发布安全门槛。
