...
| Tutor | guided/review/mistake_explanation Policy、L1/L2/L3 披露、builds-on、来源和确定性校验 Schema、ADR-0030 题图 grounding | VerifiedQuestion/AttemptEvidence/Snapshot 三重门禁、已确认 Capture 读取/哈希/图片边界、层级连续/披露差异、Provider 失败/降级、审计 | worked/blank/review 分支；有图/无图多模态载荷；L1 看懂题意、L2 方法脚手架、允许时 L3 完整讲解；含图对象缺失时 409 阻断 | L1/L2 换词重复/答案泄露、未确认状态绕过、原图 URL/对象键外发、题图忽略、错版/超纲、来源缺失、计算/单位错误、文档提示注入 | token/延迟/每级/每题和家庭预算、图片有界压缩 | `TBD` |
