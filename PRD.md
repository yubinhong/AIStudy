...
- 家庭网络和设备条件不稳定，学习记录不能依赖持续在线；已确认的作答应先保存在本机，联网后再同步。
...
- 单题拍照支持裁切、本地隐私脱敏、用户确认和多模态结构化解析；原图不外发，只有不可逆脱敏副本可发送给单一 Provider，解析结果必须人工确认。家庭可通过部署环境变量选择 Compose 内部 Qwen3.5-4B Q4_K_M；本地模式下所有当前 AI 请求统一走本地，关闭时选择已配置云端 NewAPI，不自动跨 Provider 回退。
- 断网时当前任务题号和已确认作答不丢失；完成、复习收口和跳过也以结构化事件进入受范围隔离的离线队列，恢复联网后先按批次同步 Attempt，再按顺序安全、幂等重放终态；图片、答案原文和 Session Token 不进入离线队列。
- AI、Prompt/Policy、延迟、成本和关键结果可审计，模型供应商可替换。
- 家庭可以导出和删除数据，儿童图片采用最短必要保留。
- 家长工作台的“今日需要关注”只显示上海自然日当天到期的开放错题；更早已到期的历史项不占用今日关注，完整学习记录进入独立页面，默认最近 30 个上海自然日并支持选择 180 天窗口内的单日。
...
- P1 不交付 Windows 原生客户端或 Python 游戏化模块。语文孩子端只开放“古诗抽查”和“看图写话”：古诗从家长上传并审核发布的教材逐行自动生成相邻句选择题；看图写话只提供观察、提问、句式支架和三步表达引导，不生成范文、不评分，也不复用数学题目提取链路。语文评分和复习事实仍保持确定性、追加写和版本化；英语排在语文之后，只保留 ADR-0025 的锁定可见、供应商中立、有界口语插件框架，真实 Provider 未获合规批准前不开放。
...
- 工作台的待复习区域必须显示当天到期项目的具体题干和到期日；更早到期项目保留在“学习记录”/复习入口，不显示在“今日需要关注”；“学习记录”是独立导航，默认近 30 天、可按单日筛选，不再把完整逐题详情堆叠在首页。首页不展示语文技能报告卡片，语文作答/复习事实仍按既有记录与导出合同保留。
...
- 作为孩子，当题意依赖图形、表格或其他配图时，我希望确认后的讲解页继续显示脱敏题图，以便我能对照题干思考。
...
| FR-002 | 每日任务 | Must | 区分家长安排、到期复习和系统建议；系统建议保存错题/知识点/教材来源与原因，默认须家长批准；未来计划在孩子端只读显示最近一项及其计划日，到达计划日后才可开始；未完成的过期任务可补做，不做完整课程排课。孩子端入口已恢复，每道任务题都必须携带指定题干才能开始，并将题目来源带入拍题/确认页；多题任务在同一会话内按顺序完成，端侧 SQLite 保存当前题号以支持进程重开后继续，空题任务明确阻断，不退化为通用拍题；服务端拒绝同一任务重复启动，离线终态在联网后按序重放 | 来源/审批/去重/每日上限、计划日/补做、跨题会话恢复和状态迁移测试 |
| FR-003 | 学习会话和作答 | Must | 会话关联任务；Attempt 追加写，不覆盖历史；已确认作答、任务完成和跳过在网络不可用时只写入端侧结构化队列，恢复联网后先按批次幂等同步 Attempt，再重放终态，不保存图片、答案原文或 Session Token；孩子完成或跳过题目后必须关闭讲解/确认/拍题中间页并回到学习桌 | 断网、进程重开、重连、重复提交、并发和完成后返回学习桌测试 |
| FR-004 | 数学单题拍照 | Must | 单题拍照/裁切时引导将题目和孩子答题区一并入镜；图片选择不请求无业务用途的完整照片元数据，权限被拒绝或受系统限制时必须说明具体恢复方式；App 携带 Session 只向 API 上传，API 有界流式校验后写入内部私有 MinIO；本地 OCR 仅参与敏感标签定位，并结合规则、人脸/二维码检测、元数据清除和实色遮挡生成脱敏副本；用户确认后由单一获批云视觉 Provider 分开输出结构化题目、作答步骤候选和 `worked/blank/unclear/answer_area_missing`，结果再次人工确认；已确认结果标记 `has_diagram=true` 时，当前会话的讲解页必须同时显示同一份脱敏题图和题干；不做整页全自动批改 | 四端权限拒绝/系统限制恢复、裁切/手动涂抹；API 反向越权/流式上限/文件头/尺寸/哈希/断连清理；MinIO `9000` 不暴露；云视觉固定题型、含图跳转保留、作答状态/区域覆盖、低置信度/Schema/单 Provider/删除测试 |
| FR-005 | 分模式 AI 辅导 | Must | `guided_practice/review` 先作答再给 1～3 级提示；NewAPI 可用时 L1/L2 都必须基于同一已确认题目文字、可用的已确认脱敏题图和最小教材片段生成，图中标签/数字/关系必须参与题意理解，L1 指出该题独有关系并提一个聚焦问题，L2 引用持久化 L1 并增加方法/第一步脚手架；服务端拒绝无关题型、换词重复和答案泄露，不合格时回退经固定题集验证的题型策略；`mistake_explanation` 仅在 VerifiedQuestion 和作答状态已确认后允许 L3 完整过程：`worked` 针对首个可验证错步，`blank_confirmed` 从题意/知识点开始；已确认含图但题图不可用时阻断云端解答并要求重拍；`unclear/answer_area_missing` 先重拍或手工确认 | Tutor 模式越权、含图题图传递、L2-vs-L1 递进/重复/题意相关性/答案泄露、作答状态误判/修正、Policy/Schema/来源/计算校验和固定 AI eval |
...
| FR-008 | 离线队列与同步 | Must | SQLite 缓存任务题号、会话和结构化 Attempt/终态事件/上传队列；写操作幂等；恢复联网后每批最多 50 条 Attempt，服务端返回确认后才删除，并按顺序重放完成/跳过事件；任务状态按服务端版本合并 | 飞行模式、进程终止、弱网、重复重试、部分成功和冲突测试 |
...
| FR-011 | 教材与知识点维护 | Must | 家长按孩子/数学/学年/学期/年级/教材版本上传或从受控 SmartEdu 目录选择家庭有权使用且不含个人信息的 PDF；首版 Web/OpenAPI/API 只接受 PDF，Word/PPT/Excel 明确拒绝并提示先转换。SmartEdu 只返回受限元数据，服务端按资源 ID 固定域名下载并生成同一待审核草稿；默认免配置，严格 CDN 需要登录时页面允许家长粘贴一次性 JSON（`access_token` 必填，`mac_key`/`diff` 可选），仅当前请求内存使用，不进入教材记录、日志或返回值，且不接受第三方直链。页面给出从本人 SmartEdu 登录会话获取 JSON 的具体步骤。PDF 上传不复用手工小节的教材名称：服务端先以文件名标为待识别，再只在封面/前页明确出现数学、年级和册别时本地回填教材名和册别；显式家长名称保持覆盖。服务端私有逐页渲染原页预览，以页图为主、文字层为辅按最多 4 页一批交给单一获批视觉 Provider，再对全部页级结构化观察归纳全书章节、知识点、学习目标、先修关系和带原页视觉说明的练习；空章节和缺失练习引用可安全忽略，缺少非空学习目标的知识点必须丢弃而非补造。加密/损坏/危险/超限 PDF 拒绝或隔离。家长必须查看原页并批准知识图谱，且 CurriculumSnapshot 已发布后，Tutor/任务才能引用 | PDF 扩展名/MIME/文件头/结构/哈希/幂等、非 PDF 双端拒绝、SmartEdu 资源 ID/固定域名/MAC 签名/认证/超限/镜像失败、JSON 凭据解析/输入清空/凭据不落库、封面标题识别/家长覆盖、页图/文字/扫描页、全书缺页、Schema/伪造页码或练习键、空章节/空练习引用/缺失目标过滤、解析资源/Provider 输入上限、授权和无个人信息声明、Prompt 注入、批准/发布、跨家庭和删除测试 |
| FR-012 | 提醒 | Should | 华为设备不依赖 GMS；通过 HMS 适配器或应用内提醒 | 华为设备回归与降级测试 |

SmartEdu 的页面凭据属于短期交互状态：Web 只在当前标签页 `sessionStorage` 临时保存，以便网络失败、刷新或连续选择教材；手动清除、退出登录或关闭标签页后清除。服务端以孩子/资源幂等键回放已完成导入，避免重试再次下载 PDF。
...
| FR-018 | 错题详细讲解 | Must | 拍题安全链路同时提取题目和孩子作答；VerifiedQuestion 及作答状态确认后按已发布知识范围匹配，并在可用时把同一份已确认脱敏题图作为多模态证据传给 Tutor。有作答时讲清首个可验证错步和改法；确认空白时记录“没有思路”并从题意、已知/所求、知识点开始。两者都输出逐步解法、答案校验和变式练习；已确认含图但题图缺失/校验失败、作答区缺失/不清、低置信、错版、超纲或校验失败阻断 | 端到端错题链路、有图/无图 Provider 载荷、有作答/真空白/未入镜/不清四类作答状态、人工修正、知识匹配/来源、匹配讲解、校验/阻断和成本 eval |
...
| FR-022 | 家长学习记录与保留 | Must | 工作台“今日需要关注”只列出上海自然日当天到期的开放错题；更早到期的开放错题不在首页展示但仍可从学习记录/复习入口访问。独立记录页默认查询最近 30 个上海自然日，可选择 180 天窗口内单日。API 使用带时区 UTC 半开区间，单次最多 31 天/500 条并保持 Household/Child 授权。数学记录展开详情通过家长授权的私有 Capture 媒体流显示对应拍题原图；学习记录 JSON 不包含图片字节、对象键或存储 URL，图片过期/删除时明确显示不可用。详细 VerifiedQuestion/TutorTurn 和已结束复习链路保留 180 天；开放错题及其题目不清理，Attempt/AuditEvent/账号/教材不在本策略范围 | 日期/时区/当天、过期、未来边界/上限/授权、数学图片存在/过期/删除/跨家庭与孩子角色、空状态、开放错题保护、幂等批次清理、迁移索引和生命周期日志测试 |
| FR-023 | 语文学科首版内容 | Must | 家长逐孩子启用语文；孩子端只开放“古诗抽查”和“看图写话”两项。教材分析的 `poem` 标签只作为候选，古诗必须再通过版本化目录校验标题、连续题干/正确下一句和全部可见选项，未知候选或非古诗干扰项失败关闭；通过家长授权/审核和门禁后才逐行生成相邻句选择题，同教材重发先退役旧派生题且保留历史。孩子每次点击古诗入口都重新读取当前允许题库，已退役或读取失败时不得继续打开页面内存中的旧题。答错立即显示正确下一句并进入确定性复习队列；看图写话使用独立 `picture_writing` 分析类型，只接受用户确认的脱敏派生图，返回场景观察、启发问题、句式支架和细节提示，采用“观察—说一句—补细节”三步引导，不生成范文、不评分、不复用数学题目提取链路。服务端内容仍按年级、修订、来源版本化，答案规范不下发孩子端，Attempt 追加写、Review 独立更新 | 古诗目录正反例/连续句/全选项、公开教材审核/权利凭证、旧题退役/历史保留、驻留页面刷新/撤销阻断、诗句抽题与正确答案、错误复习、Provider Schema/单 Provider/脱敏哈希、跨家庭授权、语文与数学链路隔离、Flutter 空题库/弱网/可访问性测试 |
...
| NFR-002 | 安全/隐私 | 家庭隔离、Argon2id 密码、可撤销会话、登录限速、Cookie/CSRF、最小采集、Capture 原图不外发、已确认脱敏题图可按 ADR-0030 进入 Tutor、脱敏三层门禁、单 Provider、教材无个人信息声明/页图有界批次、短期图片、导出/删除和日志脱敏 | 明文密码/原始会话、未批准跨家庭访问或 Capture 原图外发为 0；未确认/低置信度脱敏外发为 0；Tutor Provider 请求不得含原图 URL/对象键，教材 Provider 请求不得含 PDF/对象键/存储 URL/个人批注；详细学习历史固定 180 天，其他正式保留周期仍为 `TBD` | 认证/授权反向测试、凭据扫描、synthetic 脱敏泄漏测试、单题/教材 Provider 请求检查、删除演练、日志扫描 |
...
- 孩子端登录后显示当前可用学科；数学学科页只放“错题讲解、复习错题、今日任务”三个主入口。复习入口显示到期数量，今日任务显示已批准数量；复习题先显示 VerifiedQuestion 和作答区，历史答案/讲解默认隐藏；一次只展开一个学习模式。今日任务每道题都必须带指定题干，开始时把当前题干和教材来源带入拍题/确认页；同一任务的多道题按顺序使用同一会话，中间题追加作答，最后一题才结束任务；孩子选择“稍后再做”时必须以幂等 `skipped` 结果写入会话并刷新队列，空题任务不能退化为通用拍题。
...
| OPT-005 | Tutor 仅使用云端模型 | 当前家庭硬件不足以承载本地 Tutor 模型，Tutor 使用云端模型 | 已由 ADR-0028 扩展为可切换本地 Qwen/云端 NewAPI；图片解析与 Tutor 仍是两次调用，选择本地模型不等于放宽 Schema/人工确认/Policy | 本地硬件性能、模型质量、模型来源/许可证、云端 Provider 条款、预算、超时/重试、内容安全和固定 eval |
