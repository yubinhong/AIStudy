...
| FR-023 | 语文学科首版内容 | Must | 家长逐孩子启用语文；孩子端只开放“古诗抽查”和“看图写话”两项。教材分析的 `poem` 标签只作为候选，古诗必须再通过版本化的标题与连续诗句确定性目录门禁，未知候选失败关闭；通过家长授权/审核和门禁后才逐行生成相邻句选择题，同教材重发先退役旧派生题且保留历史。答错立即显示正确下一句并进入确定性复习队列；看图写话使用独立 `picture_writing` 分析类型，只接受用户确认的脱敏派生图，返回场景观察、启发问题、句式支架和细节提示，采用“观察—说一句—补细节”三步引导，不生成范文、不评分、不复用数学题目提取链路。服务端内容仍按年级、修订、来源版本化，答案规范不下发孩子端，Attempt 追加写、Review 独立更新 | 古诗目录正反例/连续句、公开教材审核/权利凭证、旧题退役/历史保留、诗句抽题与正确答案、错误复习、Provider Schema/单 Provider/脱敏哈希、跨家庭授权、语文与数学链路隔离、Flutter 空题库/弱网/可访问性测试 |
