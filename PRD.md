...
- 技术负责人：`TBD`
- 创建/更新：`2026-07-18`
- 关联任务：`TODO-003`（P0 家庭/孩子/设备纵向切片）、`TODO-012` / `PLAN-0007`（账号密码认证）、`TODO-015` / `PLAN-0013` / `ADR-0019（Proposed）`（统一孩子管理与多孩子工作台）、`PLAN-0014` / `ADR-0020` / `TODO-016`～`TODO-019`（教材驱动数学错题学习主线）
...
- 已有单家庭自用技术原型和 Ubuntu synthetic 验收，覆盖账号、任务、拍题、题目确认、最小 Tutor、周报与导出；但尚无教材驱动三入口、正式错题/复习/推荐的家庭行为数据。ADR-0020 的产品假设仍需家庭试用验证，不能把规划当作已交付需求。
...
- 家长为每个孩子设置年级、学期和教材版本，导入家庭有权使用的教材/课程文档，审核章节和知识点后发布本学期学习范围。
- 孩子端以“学科 → 数学 → 错题讲解 / 复习错题 / 今日任务”为主导航，一次只进行一种学习模式。
- 错题讲解在题目和孩子作答状态人工确认后，可以提供完整、适龄且有教材/知识点依据的解题过程：有作答时针对错步，确认空白时从头讲解；随后将题目纳入错题本与复习计划。
...
| --- | --- | --- | --- | --- |
| FR-001 | 家庭、家长、孩子档案和设备管理 | Must | 家长管理员拥有家庭空间；产品将孩子资料与登录身份作为一个管理聚合，一次事务创建 ChildProfile 与唯一绑定的孩子 Account，但认证数据与档案数据仍分表隔离；不强制手机号/邮箱；档案含年级、教材版本、科目、目标、可用时段 | 原子创建/幂等/回滚、账号—档案一对一和授权正反向测试；iPad 与 Windows Web 共享孩子档案 |
| FR-002 | 每日任务 | Must | 区分家长安排、到期复习和系统建议；系统建议保存错题/知识点/教材来源与原因，默认须家长批准；支持完成、跳过、补做，不做完整课程排课 | 来源/审批/去重/每日上限、API/客户端集成和状态迁移测试 |
...
| FR-005 | 分模式 AI 辅导 | Must | `guided_practice/review` 先作答再给 1～3 级提示；`mistake_explanation` 在 VerifiedQuestion 和作答状态已确认后给完整过程：`worked` 针对首个可验证错步，`blank_confirmed` 从题意/知识点开始；`unclear/answer_area_missing` 先重拍或手工确认；固定 JSON Schema、教材来源和可确定计算校验，模型答案不能直接成为标准答案 | Tutor 模式越权、作答状态误判/修正、Policy/Schema/来源/计算校验和固定 AI eval |
...
| FR-011 | 教材与知识点维护 | Must | 家长按孩子/数学/学年/学期/年级/教材版本导入家庭有权使用的资料；解析结果带页码/段落来源，必须审核发布不可变 CurriculumSnapshot 后才能被 Tutor/任务引用；禁止未授权内容进入公共库 | 文件/哈希/幂等、版权声明、解析/Prompt 注入、版本/发布/来源、跨家庭和删除测试 |
...
| FR-016 | 多孩子工作台选择 | Must | 家长工作台提供当前孩子选择器；问候、今日任务、当前档案和周报始终使用同一已授权 `child_id`，家庭孩子总数保持家庭级，设备未绑定孩子前保持家庭级；有效显式选择优先，其次恢复最近选择，再回退到稳定排序的首个孩子 | 两个孩子的切换/刷新/删除回退/空状态 E2E；任务与周报服务端过滤；跨家庭、兄弟姐妹和篡改 ID 反向授权测试 |
...
| FR-019 | 到期错题复习 | Must | 默认按版本化确定性 ReviewPolicy 展示今日到期队列，另可复习全部；每题先作答，正确晋级、错误重置/重新讲解，历史追加写且可重新激活 | 队列排序、1/3/7/14/30 天策略基线、时区/并发/幂等/重连、过关/重激活和无答案泄漏测试 |
| FR-020 | 可解释任务建议 | Must | TaskRecommendation 只使用已发布教材、已有练习、到期错题和有证据的薄弱知识点；默认由家长批准/修改/拒绝，首批不自动下发 AI 新编题 | 推荐依据、审批/拒绝、重复去重、每日上限、跨孩子隔离、生成失败回滚和周报解释测试 |
...
- 图片目标链路只允许 App 携带 Session 上传到 API，由 API 有界流式校验并通过内部地址写入私有 MinIO；客户端不持有存储密钥、预签名 URL、对象键或 MinIO 地址，MinIO `9000` 不向宿主/LAN 暴露。当前 `0.8.0` 仍是预签名直传，按 ADR-0018/PLAN-0012 迁移。原图默认 24 小时、旧 OCR/后续脱敏处理失败最多 7 天、裁剪题目默认 30 天；家长可保存或立即删除。临时脱敏副本在云端响应后立即删除，失败时最多 24 小时且不可长期保存。原图、MinIO URL 和对象键不得发给 Provider；删除孩子档案时级联删除相关图片。备份擦除、云 Provider 条款和真实儿童数据法域仍为 production 前置项。
...
- 孩子端登录后显示当前可用学科；数学学科页只放“错题讲解、复习错题、今日任务”三个主入口。复习入口显示到期数量，今日任务显示已批准数量；一次只展开一个学习模式。
- 错题讲解在拍题后用一个低干扰确认页同时展示题目和“识别到作答 / 答题区空白 / 不清 / 未拍到”；孩子可一键确认或修正。有作答时展示可校正步骤，确认空白后直接从头讲解；不清/未拍到时请重拍或手工选择。复习和今日任务默认隐藏答案，先要求作答。所有页面始终标明当前模式。
- 家长教材导入使用草稿 → 审核 → 发布流程，展示文件来源、版本、章节/知识点、页码定位和低置信警告；未经发布的内容不得显示为已生效。
...
- [ ] P1：到期复习逐题先作答，正确/错误按版本化确定性 Policy 更新且不覆盖历史；今日任务区分家长安排、到期复习和经批准的系统建议。
...
| 首批教材文件格式、大小/页数和扫描件处理器 | 产品 + 技术 Owner | TODO-016 OpenAPI/依赖评审前 | `ASSUMPTION`：先支持 PDF；DOCX/图片包后置，具体限制由压测批准 |
| ReviewPolicy v1 间隔与过关门槛 | 产品 Owner | TODO-018 实现前 | `ASSUMPTION`：以 1/3/7/14/30 天作为 synthetic 基线，家庭试用后确认或调整 |

## 15. 当前开发完成后的优化候选

> 来源：项目 Owner 于 2026-07-14 提供的产品设想。2026-07-18，Owner 进一步批准 ADR-0020/PLAN-0014，将教材导入、详细错题讲解、错题复习和任务建议提升为数学首科产品主线；它们不再只是后置候选，但当前仍未实现。多题分割、视频和其他项目继续按下表边界后置。

| 候选 ID | 优化方向 | Owner 设想 | 与当前基线的关系 | 进入实施前必须确认 |
| --- | --- | --- | --- | --- |
| OPT-001 | 教材/课程文档生成任务 | 后台录入本学期教材和课程文档，系统据此生成对应学习任务 | 已提升为 FR-011/FR-020、ADR-0020、PLAN-0014、TODO-016/019；当前仍未实现 | 首批 PDF 限制、解析处理器/依赖、章节/知识点审核、版权来源、版本更新和错误回滚 |
